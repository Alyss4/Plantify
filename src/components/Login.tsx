  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-primary text-white">
      ...
      <form onSubmit={handleSubmit} className="mt-6 flex flex-col space-y-4">
        ...
        {error && <p className="text-red-500 text-sm mt-2">{error}</p>} {error}

        <PlantifyForm.Button text="Se connecter" variant="primary" />
      </form>
      <Link href="/register" className="mt-4 underline">Pas encore de compte ? Inscrivez-vous</Link>
    </div>
  );
}
